# Вступительное задание в ШБР 2023, Python

## Описание

REST API-сервис, который будет регистрировать курьеров, добавлять новые заказы.

### REST API

Реализованы 7 базовых методов:

1. `POST /couriers` - для загрузки списка курьеров;
2. `GET /couriers/{courier_id}` - возвращает информацию о курьере по его идентификатору;
3. `GET /couriers` - возвращает информацию о всех курьерах;
4. `POST /orders` - для загрузки списка заказов;
5. `GET /orders/{order_id}` - возвращает информацию о заказе по его идентификатору;
6. `GET /orders` - возвращает информацию о всех заказах;
7. `POST /orders/complete` - для загрузки списка завершенных заказов.

Документация для API доступна после запуска по адресу [`http://localhost:8080/docs#/`](http://localhost:8080/docs#/).

### Рейтинг курьеров

Для получения рейтинга курьера реализован метод:

1. `GET /couriers/meta-info/{courier_id}`

### Rate limiter

В качестве Rate Limiter используется готовое решение [`asgi-ratelimit`](https://pypi.org/project/asgi-ratelimit/#built-in-auth-functions), обеспечивающее ограничение суммарной нагрузки в 10 RPS на каждый эндпоинт.

## База данных

База данных разворачивается автоматически, без использования миграций, при запуске приложения и имеет следующую структуру представленную на картинке:
<img src="./db.png">

Для работы с базой данных настроены миграции с помощью `alembic`.
